...
function parseTokenMeta(message) {
  ...
  const braceStart = message.indexOf('{', 7);
  if (braceStart === -1) return null;
  const braceEnd = message.indexOf('}', braceStart);
  if (braceEnd === -1) return null;
  const jsonStr = message.slice(braceStart, braceEnd + 1);
...
function stripTokenPrefix(message) {
  ...
  const braceEnd = message.indexOf('}', 7);
  if (braceEnd === -1) return message;
  return message.slice(braceEnd + 1).trim();
}
