...
| ⚡ **Parallel Embeddings** | `ThreadPoolExecutor` with 4 workers embeds batches concurrently — up to 4× faster than sequential |
...
│   │   ├── embeddings.py             # [NEW] AzureEmbedder — parallel batch embedding (4 workers, ThreadPoolExecutor)
...
- `AzureEmbedder` calls the Azure OpenAI Embeddings API (`text-embedding-ada-002` or any deployment) in **batches of 16**, with automatic retry and exponential backoff on rate limits.
- **Parallel batching**: batches are submitted concurrently via a `ThreadPoolExecutor` with **4 workers** (`_EMBED_WORKERS = 4`), so up to 4 × 16 = 64 texts are in-flight simultaneously. Tune `_EMBED_WORKERS` in `embeddings.py` based on your Azure tier's RPM quota.
...
| :--- | :--- | :--- |
| `TARGET_CHUNK_TOKENS` | 7,000 | Target tokens per chunk sent to LLM |
| `MAX_CHUNK_TOKENS` | 8,000 | Maximum tokens per chunk (hard limit per subchunk) |
...
| `LARGE_SOURCE_FILE_MAX_TOKENS` | 50,000 | Token cap for >3 MB source-code files (env-overridable) |
...
**Terminal 1 — direct run this command:**
```bash
 python -m uvicorn backend.main:app --host 127.0.0.1 --port 8000 
```
**FastAPI Backend (No Need):**
```bash
uvicorn backend.main:app --reload --host 0.0.0.0 --port 8000
```

**Runner Daemon(No Need):**
```bash
python -m runner.main
```

**Open in Browser:**  
Navigate to `http://127.0.0.1:8000` — the frontend is served directly by FastAPI.
