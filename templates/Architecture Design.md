...
| Field | Value |
|---|---|
| **ID** / **Version** / **Status** | `{{PROJECT_SLUG}}-ARC-001` · `{{DOC_VERSION}}` · `{{DOC_STATUS}}` |
| **Owner** / **Source** / **Style** | `{{DOC_OWNER}}` · `{{REPO_URL}}` @ `{{COMMIT_SHA}}` · {{ARCH_STYLE}} |
| **Upstream** | *`PRD.md`* → *Database*, *API*, *Deployment* |
...
## 6. ADRs, Risks & Assumptions

| ID | Decision | Status | Evidence |
|---|---|---|---|
{{#EACH adr_index}}
| `{{ADR_ID}}` | {{ADR_TITLE}} | {{ADR_STATUS}} | `{{ADR_EVIDENCE}}` |
{{/EACH}}

{{#EACH adrs}}
**`{{ADR_ID}}`.** {{ADR_CONTEXT}} → **Decision:** {{ADR_DECISION}} · +{{ADR_POSITIVE}} / −{{ADR_NEGATIVE}}
{{/EACH}}

| ID | Risk / Pattern | Score / Consequence | Evidence |
|---|---|---|---|
{{#EACH arch_risks}}
| `AR-{{INDEX}}` | {{RISK_TEXT}} | {{RISK_SCORE}} | — |
{{/EACH}}
{{#EACH patterns}}
| Pattern | {{PATTERN_NAME}} | {{PATTERN_CONSEQUENCE}} | `{{PATTERN_EVIDENCE}}` |
{{/EACH}}

| ID | Assumption / Gap | Impact / Consequence |
|---|---|---|
{{#EACH arch_assumptions}}
...
*End of `{{PROJECT_SLUG}}-ARC-001` → *`Database Design.md`*.*
